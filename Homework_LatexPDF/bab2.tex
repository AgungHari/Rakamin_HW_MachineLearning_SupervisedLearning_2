...
% Buat tinjauan pustaka untuk SelectKBest gunakan refrensi yang relevan
SelectKBest adalah metode pemilihan fitur yang digunakan untuk memilih sejumlah fitur terbaik dari dataset berdasarkan skor statistik tertentu. Metode ini membantu dalam mengurangi dimensi data dengan memilih fitur-fitur yang paling relevan untuk model, sehingga meningkatkan kinerja dan interpretabilitas model. SelectKBest dapat digunakan dengan berbagai fungsi skor, seperti ANOVA F-value, chi-squared, atau mutual information.

Adapun penggunaan SelectKBest harus menggunakan data yang sudah dibagi menjadi data latih dan data uji. Hal ini penting untuk memastikan bahwa fitur yang dipilih tidak bias terhadap data uji, sehingga model yang dibangun dapat generalisasi dengan baik pada data baru.

\begin{equation}
    \text{Score}(X, y) = \frac{1}{n} \sum_{i=1}^{n} f(X_i, y)
\end{equation}
di mana:
\begin{itemize}
    \item \( \text{Score}(X, y) \) adalah skor fitur yang dihitung berdasarkan fungsi skor \( f \).
    \item \( n \) adalah jumlah fitur yang ada.
    \item \( X_i \) adalah nilai dari fitur ke-i.
    \item \( y \) adalah label target (kelas) yang ingin diprediksi.
    \item Fungsi skor \( f \) dapat berupa ANOVA F-value, chi-squared, atau mutual information, tergantung pada jenis data dan tujuan analisis.
    \item Nilai skor yang lebih tinggi menunjukkan bahwa fitur tersebut lebih relevan untuk memprediksi label target.
\end{itemize}